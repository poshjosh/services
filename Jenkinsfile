        ...
        stage('All') {
            agent {
                dockerfile {
                    ...
                    args '-v "$(which docker)":/usr/bin/docker -v /usr/bin/docker:/usr/bin/docker -v /root/.m2:/root/.m2 -v /var/run/docker.sock:/var/run/docker.sock -v "$PWD":/usr/src/app -v "$HOME/.m2":/root/.m2 -v "$PWD/target:/usr/src/app/target" -w /usr/src/app'
