        ...
        stage('All') {
            agent {
                dockerfile {
                    ...
                    args '-v "C:/Program Files/Docker/Docker/resources/bin:/usr/bin/docker".ToLower() -v /root/.m2:/root/.m2 -v /var/run/docker.sock:/var/run/docker.sock -v "$PWD":/usr/src/app -v "$HOME/.m2":/root/.m2 -v "$PWD/target:/usr/src/app/target" -w /usr/src/app'
